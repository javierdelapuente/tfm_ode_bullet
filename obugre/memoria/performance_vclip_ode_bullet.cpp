...
int main( int argc, const char* argv[] )
{
    ...
    //EngineLogger engine_logger{"performancevclipodebullet.csv"};
    ...
    odevclipengine.set_disable_vclip_cache(false);
    ...
    bulletvclip.set_margin(margin);
    bulletvclip.set_use_vclip(true);
    //bulletvclip.add_listener(engine_logger);
    ...
    // el orden es motivo de trampa.
    // alterno ode y bullet para que las cachés de la cpu
    // se vacíen un poco. Aún así tiene una ventaja
    // marginal los últimos. En cualquier caso,
    // V-Clip es superior en ambos casos.
    ...
    looper.add_engine("odevclip", Vector{ -50, 0, 50 });
    ...
    for (int i = 0; i < number_objects/objects_per_second; i++)
    {
        ...
        for (int i = 0; i < objects_per_second; i++)
        {
            ...
            system.insert_body_in_engine(body.get_name(), "bulletvclip");
        }
        if (looper.is_end_loop()) break;
    }
    looper.loop(real_seconds{20.0});
