...
void MovableText::setFontName(const String &fontName)
{
    ...
    if (mFontName != fontName || !mpMaterial || !mpFont)
    {
        ...
        if (!mpFont) {
          mpFont = FontManager::getSingleton().create(mFontName, ResourceGroupManager::INTERNAL_RESOURCE_GROUP_NAME).get();
          mpFont->setType(FontType::FT_TRUETYPE);
          mpFont->setTrueTypeSize (20);
          mpFont->setSource("cuckoo.ttf");
          //std::cerr << "No existed la fuente " << fontName << std::endl;
